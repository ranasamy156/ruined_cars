      ...
      <div class="col-lg-4">
        ...
        <div class="row">
          ...
          <div class="row">
            ...
            <?php 
            if($row['inv_status'] == 0){ 
              if($row['amana_status'] == 2 || $row['traffic_status'] == 2 || $row['vendor_status'] == 2){
            ?>
            <div class="col-sm-2">
            </div>
            <div class="col-sm-1">
            ...
            <?php
              if(isset($_POST['sendpro'])){
                $db5 = new database();
                $messagepro = "تم تغيير حالة الطلب رقم ".$_GET['r']."من قبل البحث الجنائي";
                  if($_POST['procedures'] == 1){
                    $li = $db5->RunDML("update lifting_procedures set inv_status = '1' where id=".$_GET['n']);
                    if($row['traffic_status'] == 1 && $row['amana_status'] == 1 && $row['vendor_status'] == 1){
                    ...
                    $st = $db5->RunDML("update request set status_id = '28', reason = '".$_POST['reason']."' where id=".$_GET['r']);
                    $li = $db5->RunDML("update lifting_procedures set inv_status = '2' where id=".$_GET['n']);
                  }

                $msg2 = $db5->RunDML("insert into notification values (Default, '".$_GET['r']."' , '".$messagepro."' , '0' , '2')");
