      ...
      <div class="col-lg-4">
        ...
        <div class="row">
          ...
          <div class="row">
            ...
            <?php 
            if($row['inv_status'] == 0){ 
              if($row['traffic_status'] == 2 || $row['vendor_status'] == 2 || $row['amana_status'] == 2){
            ?>
            <div class="col-sm-2">
            </div>
            <div class="col-sm-1">
              ...
              if(isset($_POST['sendpro'])){
                ...
                $messagepro = "تم تغيير حالة الطلب رقم ".$_GET['r']."من قبل البحث الجنائي";
                  ...
                  if($_POST['procedures'] == 1){
                    $sql = "CALL changeInvStatus1(?)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bindParam(1, $liftID, PDO::PARAM_INT);
                    $stmt->execute();

                    if($row['traffic_status'] == 1 && $row['inv_status'] == 1 && $row['vendor_status'] == 1){
                      $sql = "CALL changeInvStatus2(?)";
                      $stmt = $conn->prepare($sql);
                      $stmt->bindParam(1, $reqID, PDO::PARAM_INT);
                      $stmt->execute();
                      
                    }else{
                      $sql = "CALL changeInvStatus3(?)";
                      $stmt = $conn->prepare($sql);
                      $stmt->bindParam(1, $reqID, PDO::PARAM_INT);
                      $stmt->execute();
