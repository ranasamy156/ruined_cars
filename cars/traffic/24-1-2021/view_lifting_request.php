              ...
              if(isset($_POST['sendpro'])){
                ...
                $messagepro = "تم تغيير حالة الطلب رقم ".$_GET['r']."من قبل المرور";
                  if($_POST['procedures'] == 1){
                    $li = $db5->RunDML("update lifting_procedures set traffic_status = '1' where id=".$_GET['n']);
                    if($row['amana_status'] == 1 && $row['inv_status'] == 1 && $row['vendor_status'] == 1){
                      $st = $db5->RunDML("update request set accepted_at = NOW(), ready_to_close = '1', status_id = '13' where id =".$_GET['r']);
                    }else{
                      $st = $db5->RunDML("update request set status_id = '32' where id =".$_GET['r']);
                    }
                  }elseif($_POST['procedures'] == 2){
                    $st = $db5->RunDML("update request set status_id = '27', reason = '".$_POST['reason']."' where id=".$_GET['r']);
                    $li = $db5->RunDML("update lifting_procedures set traffic_status = '2' where id=".$_GET['n']);
                  }

                $msg2 = $db5->RunDML("insert into notification values (Default, '".$_GET['r']."' , '".$messagepro."' , '0' , '3')");
                $msg3 = $db5->RunDML("insert into notification values (Default, '".$_GET['r']."' , '".$messagepro."' , '0' , '2')");
