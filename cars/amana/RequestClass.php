    ...
    class Requests extends Database implements Operation{
       ...
       var $user_id; var $created_at; var $status_id;
        ...
        public function update(){
            $m=parent::RunDML("update request set status_id='11' where id='".$_GET["id"]."' ");
            return $m;
        }
        ...
        public function SearchFunction(){
            $rs=parent::GetData("select rq.* ,st.description as status_name ,us.name from request rq ,statuses st, users us where (rq.plate_number like '%".$_GET["n"]."%' or rq.en_platenum like '%".$_GET["n"]."%') and rq.user_id = us.id and rq.status_id=st.id");
            return $rs;
        }
        public function GetSearch(){
           ...
           // print_r($mysqldate);
            ...
            where (rq.created_at between '".$_GET["f"]."' and '".$_GET["t"]."' or rq.plate_number like '%".$_GET["n"]."%' or rq.en_platenum like '%".$_GET["n"]."%' or rq.plate_number like '%".$reverse."%' or rq.en_platenum like '%".$reverse."%' or rq.id like '%".$_GET["n"]."%' or rq.chassis like '%".$_GET["n"]."%' or ct.name like '%".$_GET["n"]."%' or ar.name like '%".$_GET["n"]."%') and rq.model_id=model.id and model.manufacture_id=man.id and rq.user_id = us.id and rq.city_id=ct.id and rq.area_id =ar.id and rq.state_id=st.id and rq.status_id=sts.id");
